...
_template_list = [
    np.array(shape, dtype=np.float32).reshape(-1)
    for shape in CHARACTER_SHAPES
]
TEMPLATES = np.stack(_template_list, axis=0)             # shape (n_shapes, 9)
_t_means   = TEMPLATES.mean(axis=1, keepdims=True)       # (n_shapes,1)
_t_stds    = TEMPLATES.std(axis=1, keepdims=True) + 1e-5 # avoid div0
NORMALIZED_SHAPES = (TEMPLATES - _t_means) / _t_stds     # (n_shapes,9)
...
class CustomImage:
    ...
    def to_color_shape_map(self, target_width:int= -1, target_height:int= -1, downscale_method:int= cv2.INTER_LINEAR_EXACT):
        ...
        # 1) downscale source to 3× resolution
        self.downscale(target_width * 3, target_height * 3, downscale_method)
        h, w, _ = self.array.shape
        grid_rows, grid_cols = h // 3, w // 3

        # 2) extract all 3×3×3 patches
        grids = np.lib.stride_tricks.as_strided(
            self.array,
            shape=(grid_rows, grid_cols, 3, 3, 3),
            strides=(self.array.strides[0] * 3,
                        self.array.strides[1] * 3,
                        *self.array.strides)
        )

        # 3) average RGB for ANSI color
        grid_colors = np.mean(grids, axis=(2, 3)).astype(np.uint8)  # shape (rows, cols, 3)

        # 4) build N×9 patch matrix (3×3 grayscale → 9 floats)
        gray_patches = np.mean(grids, axis=4).astype(np.float32)   # (rows,cols,3,3)
        patches     = gray_patches.reshape(grid_rows * grid_cols, 9)  # (N,9)

        # 5) normalize each patch to zero‑mean & unit‑variance
        p_means     = patches.mean(axis=1, keepdims=True)             # (N,1)
        p_stds      = patches.std(axis=1, keepdims=True) + 1e-5       # (N,1)
        norm_patches= (patches - p_means) / p_stds                     # (N,9)

        # 6) compare against our pre‑normalized templates
        #    NORMALIZED_SHAPES was built above to be (n_shapes,9)
        dists = cdist(norm_patches, NORMALIZED_SHAPES, metric='euclidean')  # (N,n_shapes)
        best  = dists.argmin(axis=1).reshape(grid_rows, grid_cols)         # (rows,cols)

        # 7) downsample back to target grid
        row_idx = (np.arange(target_height) * (grid_rows / target_height)).astype(int)
        col_idx = (np.arange(target_width ) * (grid_cols / target_width )).astype(int)
        idx_map = best[row_idx[:, None], col_idx]                     # (target_h, target_w)
        col_map = grid_colors[row_idx[:, None], col_idx]             # (target_h, target_w, 3)

        # 8) build and return your colored CharacterMap
        cmap = CharacterMap(width=target_width, height=target_height)
        chars = np.array(OPAS, dtype=object)[idx_map]
        ansi  = [
            f"\033[38;2;{r};{g};{b}m"
            for r, g, b in col_map.reshape(-1, 3)
        ]
        cmap.array = (np.array(ansi)
