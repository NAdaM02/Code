...
###
    ...
    def set_action_selector_for_key_dict():
        global key_action_dict
        key_action_dict = {
            'w' : increase_volume,                Key.up : increase_volume,
            's' : decrease_volume,                Key.down : decrease_volume,
...
###
    ...
    def update_album_cover(album_cover_array):
        if current:
            if album_cover_array is not None:
...
def spotify_interact_functions():
    ...
    global adjust_volume, decrease_volume, increase_volume
...
###
    ...
    def search_tracks(query:str, limit=8):
        ...
        if query:
            results = sp.search(q=query, type='track', limit=limit+1)['tracks']['items']
...
def main_loop():
    global sp, current, current_time, song_length, progress, liked_status, time_since_last_sync, album_cover_array, DEVICE_ID, volume

    last_request_time, last_calculated_time, lyrics, playing_status, previous_name = (precise_time()-request_buffer-0.1, 0, "", False, "")
    ...
    while True:
        try:
            if request_buffer< precise_time() - last_request_time:
                current = sp.current_playback()
