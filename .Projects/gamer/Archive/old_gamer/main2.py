...
threshes_df = pd.DataFrame([{'dir':"",'thresh':0}])




def findImageOnScreen(img_to_detect_path:str, screen_index:int=0, threshold:float=0.78, memory:bool=True, save_val=True) :
    global last_val, threshes_df
    ...
    if max_val >= threshold:
        ...
        if not threshes_df['dir'].eq(img_to_detect_path).any():
            new_row = pd.DataFrame( [{'dir':img_to_detect_path, 'thresh':max_val}] )
            threshes_df = pd.concat( [threshes_df, new_row], ignore_index=True )
...
def selectAnswer(i:int) :
    ...
    box = False
    while not box:
        box = findImageOnScreen(q_pics[i], threshold=0.5)
    if mistake:
...
try:
        ...
        while (mode == "goal" and _sum < goal) or (mode == "time" and (time.time() - start_time) < time_goal):
            ...
            if difficulty == 'hard' or difficulty == 'h' :
                ...
                while not found_hard:
                    found_hard = findImageOnScreen((f'{dot}/Pics/hard.png'), threshold=0.5)
                wait(0.1)
                ...
                while not found_medium:
                    found_medium = findImageOnScreen((f'{dot}/Pics/medium.png'), threshold=0.5)
                wait(0.1)
            ...
            while not found_start :
                found_start = findImageOnScreen(f'{dot}/Pics/start.png', threshold=0.5)
            ...
            while found_l_for <= 0.1 :
                found_l_n = findImageOnScreen(f'{dot}/Pics/l.png', threshold=0.5)
                if not found_l :
                    found_l_for = 0
                    if found_l_n :
                        found_l_for = 0
                ...
                found_start_n = findImageOnScreen(f'{dot}/Pics/start.png', threshold=0.5)
                if not found_start :
                    found_start_for = 0
                    if found_start_n :
                        found_start_for = 0
                ...
                while not found_t:
                    ...
                    for i in order: # i = [1, 2 ... q_num-1, 0]
                        if i not in solved:
                            print(f"\b{i+1}", end="")
                            sys.stdout.flush()
                            found = findImageOnScreen(t_pics[i],threshold=thresh)
                            if found:
                                m = selectAnswer(i)
                                found_t = True
                                solved.append(i)
            ...
            wait(1.5)
            err = False
            while not findImageOnScreen(f'{dot}/Pics/new.png', threshold=0.5):
                err = True
                for i in order: # i = [1, 2 ... q_num-1, 0]
                    print(f"\b+{i+1}", end="")
                    sys.stdout.flush()
            ...
            while score == 0:
                thresh -= 0.01
                if findImageOnScreen(f'{dot}/Pics/+8.png', threshold=thresh):
                    score = +8
                elif findImageOnScreen(f'{dot}/Pics/+4.png', threshold=thresh):
                    score = +4
                elif findImageOnScreen(f'{dot}/Pics/+4.png', threshold=thresh):
                    score = -4
                    loss_count += 1
            ...
            found_new = False
            while not found_new:
                found_new = findImageOnScreen(f'{dot}/Pics/new.png', threshold=0.5)
            wait(0.1)
            while found_new:
    ...
    threshes_df.head()
    threshes_df.to_csv('threshes.csv', index=False)
    print('\n'*3,"saved")
